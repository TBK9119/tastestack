...
export type CatalogItem = {
  apiId: string; type: MediaType; title: string; year: string;
  creator: string; progressTotal: number; cover: string;
...
export function getCatalogItem(apiId: string) {
  return CATALOG.find((item) => item.apiId === apiId);
}

export function catalogSourceForType(type: MediaType): "anilist" | "tmdb" | "rawg" | "lastfm" | "openlibrary" {
  if (type === "anime" || type === "manga") return "anilist";
  if (type === "book") return "openlibrary";
  if (type === "game") return "rawg";
  if (type === "album") return "lastfm";
  return "tmdb";
}
